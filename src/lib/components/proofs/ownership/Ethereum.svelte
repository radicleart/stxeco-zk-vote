    ...
    async function connectAndSign() {
        ...
        if (typeof window.ethereum !== 'undefined') {
            try {
                ...
                dispatch('signedMessage', {address, signature, chain: CHAIN.ETHEREUM, wallet: WALLET.METAMASK})
            } catch (error) {
      ...
      <p class="text-sm font-thin mb-5">
          Connect with Meta Mask, Rainbow, Zengo
      </p>
      <p class="text-sm font-thin">
          <button on:click={() => connectAndSign()} class="bg-primary-500 hover:bg-primary-600 text-black font-semibold py-2 px-6 rounded transition duration-200 disabled:bg-gray-800 disabled:text-gray-300 disabled:cursor-not-allowed">
              Sign message
            </button>
          </p>
      </div>
